...
na temat 1000 użytkowników z różnych środowisk oraz finalną decyzję o kupnie roweru. Zbiór ten może posiadać braki, wartości odstające oraz inne niedoskonałości.
...
kod tworzy tabelę pokazującą rozkład ilości NA na poszczególne zmienne oraz ich procent w stosunku do całego zbioru.
...
Powyżej jeszcze wykres słupkowy pokazujący liczbę
...
Cars_NA, Children_NA i Age_NA (tj. poziome słupki). Opis poszczególnych kolumn wygląda następująco:


- kolumna 1: jest dokładnie 9 obserwacji, w których zmienna ma wartość NA
- kolumna 2: jest dokładnie 7 obserwacji, w których zmienna Children ma wartość NA 
- kolumna 3: jest dokładnie 6 obserwacji, w których zmienna Age ma wartość NA 
- kolumna 4: jest dokładnie 5 obserwacji, w których zmienna Income ma wartość NA 
- kolumna 5: jest dokładnie 1 obserwacja, w której zmienna Income, jak i Age ma wartość NA 
...
Zacznijmy od zbadania zależności zmiennej cars. Wyznaczymy wykres zależności w stosunku do zmiennych Income
oraz Age.
...
Biorąc pod uwagę, że zmienne Martial.Status i Gender, to zmienne jakościowe, cieżko jest policzyć statystyki opisowe, ponieważ dane nie są liczbowe. Można natomiast zauważyć iż w zmiennej martial.status dominują osoby będące w związku małżeńskim, które stanowią niemal 54% populacji.
...
```
W badanej próbie dominują osoby z małą liczbą dzieci (0–2), co stanowi około 66% respondentów, z czego 27,6% nie posiada dzieci.
...
Ho: zmienna income ma średnią wartość w populacji wynoszącą 60000 USD
Ha: zmienna income nie ma średniej wartości w populacji wynoszącej 60000 USD 
